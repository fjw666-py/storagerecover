...
//		模拟一个不会持久写入特定密钥的存储管理器
//		不管恢复是否正确实现，只要完成此操作，测试就会成功。
    ...
    public void TestRecoveryPerformance() {
		...
		System.out.println("lvalue_last :" + lvalue);
   ...
   public void TestRecoveryPerformance2() {
		...
		for(int i = 0; i < nTXNs;) {
	   		...
	   		int nToInterleave = r.nextInt(5) + 3; //得到一个范围在5-8之间的数值
	   		...
	   		try {
		   		...
		   		for(int j = 0; j < nToInterleave; j++) {
		   			long TXid = i + j;
		   			boolean shouldAbort = r.nextBoolean() && (j != nToInterleave - 1); //最后一个事务一定是提交的，前面的可能提交，也可能不提交
		   			if (shouldAbort) {
		   				tm.abort(TXid);
		   			} else {
		   				tm.commit(TXid);
    ...
    public void TestRepeatedFailuresTemplate(final Random seeds, double failureRate) {
		...
		for(int trial = 0; trial < nTrials; trial++) {
			...
			tm.start(TXid);
			//crash之后不一定会进行abort
			//第一条trial一定是false的，一定不会shouldCrash 和shouldAbort
...
//
				if (shouldAbort) {
					tm.abort(TXid);  //可能是从startnoend和starttx中删除
				} else {
					tm.commit(TXid); //也有可能会加入到已经commit的事务集中去
				}
			} catch (CrashException e) {
			...
			if (shouldCrash) {
				...
				if (r.nextBoolean()) {
					sm.do_persistence_work(); //没有将dont_persist_key中的key进行持久化操作
				}

				sm.crash(); //只留下了被持久化的key,同时persisted_listener也被清空
				//Resume processing:
				lm.resumeServingRequests();
				sm.blockPersistenceForKeys(null); //将dont_persist_key清空
				tm = new TransactionManager();
				sm.setPersistenceListener(tm);
				sm.in_recovery = true;
				tm.initAndRecover(sm, lm);
...
//				if(entry.getKey() == 0 && trial ==11) System.out.printf("trial =%d,最后一次添加0到lastCommittedValues中时候，key=0对应的值%s \n",trial,Arrays.toString(read).substring(0,10));
